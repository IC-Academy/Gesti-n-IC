...
async function obtener(id: string, rol?: Rol, usuarioId?: string): Promise<ServiceResult<Actividad>> {
  const db = getDb()
  const a = db.actividades.find((a) => a.id === id)
  if (!a) return delay({ ok: false, error: { code: 'NO_ENCONTRADA', message: 'Actividad no encontrada.' } })
  const esGestor = can(rol, 'actividades.editar_cualquiera')
  const esAsignado = Boolean(usuarioId && (a.responsableId === usuarioId || a.equipoIds.includes(usuarioId)))
  if (!esGestor && !esAsignado) {
    return delay({ ok: false, error: { code: 'SIN_PERMISO', message: 'Esta actividad no está asignada a tu usuario.' } })
  }
  return delay({ ok: true, data: a })
}
